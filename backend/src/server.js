...
const customerChatbotRoutes = require('./routes/customerChatbotRoutes');
...
const allowedOrigins = [
  ...
  'http://localhost:3000',
  'http://localhost:3001',
  'https://Smart Kirana.onrender.com',
...
app.get('/health', (req, res) => {
  res.status(200).json({
    success: true,
    message: 'Smart Kirana Backend Server is running',
...
app.use('/api/chatbot/customer', customerChatbotRoutes); // Customer-side AI chatbot
app.use('/api/sync', dataSyncRoutes); // Data synchronization between stores and customers
...
app.get('/', (req, res) => {
  res.status(200).json({
    success: true,
    message: 'Welcome to Smart Kirana API',
    version: '1.0.0',
    endpoints: {
      health: '/health',
      auth: '/api/auth',
...
app.listen(PORT, () => {
  console.log('🚀 Smart Kirana Backend Server Started');
